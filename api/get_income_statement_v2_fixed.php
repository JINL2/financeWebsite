<?php
/**
 * Income Statement API V2 - Fixed to use RPC
 * Fixed version that properly calls rpc_get_income_statement_v2
 */

// 디버그 로그 추가
error_log("=== API CALLED: get_income_statement_v2_fixed.php ===");
...
try {
    ...
    // Get income statement data using RPC function
    error_log("Step 3: Getting income statement data via RPC");
    $income_statement = get_income_statement_via_rpc($params);
 ...
 * Get income statement data using RPC function
 */
function get_income_statement_via_rpc($params) {
    error_log("=== Calling RPC Function ===");
    error_log("Company ID: " . $params['company_id']);
    error_log("Date range: " . $params['start_date'] . ' to ' . $params['end_date']);
    error_log("Store ID: " . ($params['store_id'] ?: 'null'));
    ...
    try {
        ...
        try {
            ...
            error_log("Returning empty result set due to RPC failure");
            $raw_data = [];
        }
        ...
        error_log("Error in get_income_statement_via_rpc: " . $e->getMessage());
        throw $e;
    }
}
...
function process_income_statement_data_v2($raw_data) {
    ...
    if (empty($raw_data)) {
        error_log("No raw data to process - returning empty structure");
        return $structured;
    }
    
    // Group data by section and process
    $sections = [];
    foreach ($raw_data as $row) {
        ...
        if (!isset($sections[$section][$category])) {
        ...
        $amount = safe_float($row['net_amount']);
        
        // Skip subtotal rows for item processing
        if (!$row['is_subtotal']) {
            $sections[$section][$category]['items'][] = [
        ...
        $sections[$section][$category]['subtotal'] += $amount;
        $sections[$section][$category]['transaction_count'] += safe_int($row['transaction_count']);
    }
    
    // Map sections to structured format
    foreach ($sections as $section_name => $section_data) {
        foreach ($section_data as $category_key => $category_data) {
            switch ($section_name) {
                case 'revenue':
                ...
                case 'non_operating':
                    $structured['non_operating']['expenses'][$category_key] = $category_data;
                    $structured['non_operating']['net_total'] += $category_data['subtotal'];
                    break;
                    
                case 'tax':
                    $structured['tax']['categories'][$category_key] = $category_data;
                    $structured['tax']['total'] += $category_data['subtotal'];
                    break;
                    
                case 'comprehensive_income':
                    $structured['comprehensive_income']['categories'][$category_key] = $category_data;
                    $structured['comprehensive_income']['total'] += $category_data['subtotal'];
                    break;
            }
        }
    }
...
function callSupabaseRPC($function, $params) {
    ...
    $url = 'https://atkekzwgukdvucqntryo.supabase.co/rest/v1/rpc/' . $function;
    
    $headers = [
        'apikey: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImF0a2VrenduZ3VrZHZ1Y3FudHJ5byIsInJvbGUiOiJhbm9uIiwiaWF0IjoxNzQyODk0MDIyLCJleHAiOjIwNTg0NzAwMjJ9.G4WqAmLvQSqYEfMWIpFOAZOYtnT0kxCxj8dVGhuUYO8',
        'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImF0a2VrenduZ3VrZHZ1Y3FudHJ5byIsInJvbGUiOiJhbm9uIiwiaWF0IjoxNzQyODk0MDIyLCJleHAiOjIwNTg0NzAwMjJ9.G4WqAmLvQSqYEfMWIpFOAZOYtnT0kxCxj8dVGhuUYO8',
        'Content-Type: application/json',
        'Prefer: return=representation'
    ];
    
    error_log("Calling RPC URL: " . $url);
    error_log("RPC Parameters: " . json_encode($params));
    ...
    error_log("RPC HTTP Code: " . $httpCode);
    error_log("RPC Response: " . substr($response, 0, 500));
